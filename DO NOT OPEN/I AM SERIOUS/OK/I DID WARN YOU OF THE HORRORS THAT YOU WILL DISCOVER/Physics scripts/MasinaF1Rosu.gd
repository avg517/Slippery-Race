...
func _ready() -> void:
	cam = $Camera2D  # Adjust the path to match your scene tree
...
var torq2 = [20.0,10.0,7.6,6.4,5.6,4.8,2.0]
...
>>>>>>> parent of f2b6fdc (gearbox 1600)
		...
		if(int(speed.length()/4)>=gears2[gear] and gear!=6):
			gear+=1
		if(int(speed.length()/4)<=gears2[gear-1] and gear!=1):
			gear-=1
